# Objetivo

Trabalho de refatoração feito na UC de Gestão e Qualidade de Software. O principal objetivo do trabalho foi refatorar um código legado aplicando boas práticas do Clean Code, como 
SOLID, DRY, KISS, etc.

# Como rodar

Baixe o repositório e abra o projeto no NetBeans. Importe o banco de dados dentro da pasta database no MySQL.

# Vídeo

Link: https://youtu.be/pVXC5UQZksA
A apresentação principal do trabalho foi feita neste vídeo, seguindo as especificações propostas pelo docente Luiz Henrique Zanin.


# Integrantes

Andrey Ferreira Pichuti
João Gabriel Silva Barbara da Conceição - GitHub: https://github.com/joao-gabriel-barbara
Moshe Achkiy Mandujano Silverio - GitHub: https://github.com/Achkiy
